...
import { useAppState } from '../../app/App';
  ...
  import { FaUserAlt, FaLock, FaEnvelope,FaBook } from "react-icons/fa";
  import axios from "axios"
  const CFaUserAlt = chakra(FaUserAlt);
  const CFaLock = chakra(FaLock);
  const CFaEnvelope = chakra(FaEnvelope);
  const CFaBook = chakra(FaBook);

function EducateurSignup() {
     ...
     const handleSubmit = async (event) => {
       ...
       try {
         const response = await axios.post('/educateur/signupEducateur/', formObject, { withCredentials: true });
          ...
          <Stack spacing={4} p="1rem" backgroundColor="whiteAlpha.900" boxShadow="md">
            ...
            <FormControl>
              <InputGroup>
                <InputLeftElement
                  pointerEvents="none"
                  children={<CFaBook color="gray.300" />}
                />
                <Input name='module' type="text" placeholder="Subject" value={module} onChange={(e) => setModule(e.target.value)} />
              </InputGroup>
            </FormControl>


            <Button borderRadius={10} type="submit"  variant="solid" bg="teal.300" color="white" width="full">
              Signup
            </Button>
...
export default EducateurSignup